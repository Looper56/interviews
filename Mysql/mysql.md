...
##### 为什么不是一般二叉树
 二叉树特殊化为一个链表，相当于全表扫描，平衡二叉树相对二叉树来说查找效率更稳定
##### 为什么不是平衡二叉树
 平衡二叉树每个节点只存储一个键值和数据，B树存储更多节点，树的高度低，磁盘io次数小
##### 为什么不是B树而是B+树
 B+树非叶子节点不存储数据，只存储键值，B树节点不仅存储键值，也会存储数据
 InnoDB中页的默认大小时16kb，如果不存储数据，就能存储更多的键值，相应树的阶数就会更大，树越胖越矮，磁盘IO减少
 B+树索引的所有数据均存储在叶子节点，而且数据是按照顺序排列，链表连着，B+树使得范围查找，顺序查找，分组以及去重变得简单
##### FULL-TEXT全文索引
...
##### 推荐分表分库大小
 常说Mysql每张表最好不要超过2000W条数据
 阿里开发手册：单表行数超过500W或单表容量超过2GB推荐分库分表
 实际情况每张表自身字段不同，字段所占空间不同
 聚簇索引和非聚簇索引：除格式信息以外，索引叶子节点只存索引数据，比如索引为id，那非叶子节点就是id数据
 如果数据量大，B+树变成4层查询，4次磁盘IO，性能下降，所以计算`InnoDB的3层B+树最多可以存放多少条数据`
 Mysql每个节点大小默认16kb，最大为64kb，最小为4kb
 考虑分表考虑实际表情况，各表字段支持大小总和才是考虑因素
 [Mysql因大小或条数分表分库参考](https://juejin.cn/post/7165689453124517896)
...
##### Mysql负优化
 `LIMIT` 与 `ORDER BY` 是特殊组合，尤其`ORDER BY`中存在`BTREE`索引情况下
 表现形式：普通的查询是根据条件进行筛选，然后在结果集中排序，然后获取LIMIT条数的数据，但是在具备上述条件的特殊sql中执行逻辑是这样的，根据ORDER BY字段的B+树索引来查找满足条件的数据，直到凑满LIMIT设定的数值为止，这就存在一个问题，在结果集中的数据大于LIMIT的场景下，这个性能固然是非常棒的，但是如果最后的结果集中的数据小于LIMIT，就会存在永远凑不满的情况，所以最终这个MySQL的性能优化就会变成全表扫描的“负优化”。